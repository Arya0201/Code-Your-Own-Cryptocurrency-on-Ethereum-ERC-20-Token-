...
contract('AryaToken', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return AryaToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'Arya Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'Arya', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard){
      assert.equal(standard,'Arya Token v1.0','has the correct standard')
    });
  })
  ...
  it('handles delegated token transfers',function(){
    return AryaToken.deployed().then(function(instance){
      ...
      assert(error.message.indexOf('revert')>=0,'can not transfer value larger than approved amount');
      ...
      assert.equal(balance.toNumber(),10,'add the amount from receiving account');
